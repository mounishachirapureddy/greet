...
resource "aws_eks_cluster" "messaging-eks" {
  name     = "${var.environment_name}-messaging-cluster"
...
resource "null_resource" "install_istio" {
  depends_on = [aws_eks_cluster.messaging-eks]

  triggers = {
    cluster_id = aws_eks_cluster.messaging-eks.id
  }

  provisioner "local-exec" {
    command = <<-EOT
      aws eks update-kubeconfig --name prod-messaging-cluster --region ap-south-1
